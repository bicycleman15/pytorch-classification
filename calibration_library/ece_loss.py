...
class ECELoss(torch.nn.Module):
    ...
    def forward(self , output, target):
        ...
        # output = [batch, n_class] np array now 
        predicted_score, predicted_label = output.max(dim = 1)
        # predicted_label [batch] np array now 
        ...
        overall_EceLoss = torch.sum(torch.abs(avg_acc - avg_conf) * (current_no_pred_tot/torch.sum(current_no_pred_tot)))
